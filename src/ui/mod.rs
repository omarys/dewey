...
pub fn render(f: &mut Frame, app: &mut App) {
    ...
    if is_portrait {
        ...
        if area.width >= 130 {
            // Wide Desktop 3-Column Layout: Series | Chapters | Full Details & Stats
            let cols = Layout::default()
                .direction(Direction::Horizontal)
                .constraints([
                    Constraint::Percentage(33), // Series List
                    Constraint::Percentage(38), // Chapters List
                    Constraint::Percentage(29), // Details Pane
                ])
                .split(body);

            let series_rect = cols[0];
            let chapters_rect = cols[1];
            let details_rect = cols[2];
    ...
    } else if app.input_mode == crate::app::InputMode::EditSeries {
        render_edit_series_modal(f, app, &theme);
    }
